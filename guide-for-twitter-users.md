...
X（旧Twitter）を使っている、あるいは使っていた。
でも最近、なんとなく疲れる・見たいものが見つからない・居心地が変わった……と感じている人は多いと思います。

- 最近、Bluesky（ブルースカイ）というSNSの名前を聞くことがあるけど、どんなSNSなの？
- Blueskyのアカウントを作ってはみたけど、まだどういうことができるのか良くわからないんだけど……
- 「保存」「青チェック」「連絡先から友だちを探す」など新機能が増えたらしいけど、結局どう使えばいいの？

そんな人向けのガイドです。
「Xから完全移住！」という話ではなく、「選択肢を増やす」ためのガイドだと思ってください。

文字ばっかりで長くてすみません。必要なところだけ拾い読みでOKです。
...
- [ソラタマ for Android](https://bsky.softama.com/) - タブブラウザ型のAndroid用アプリ
...
X、Instagram、YouTubeなど、１つの企業・グループによって運用されているようなサービスの場合は「```@tkusano```」のように、「```@```」の後に１つの単語がくっついたものが使われています。
...
- ``@sega.jp`` : セガ（ゲームメーカー） [&#x2197;](https://bsky.app/profile/sega.jp)
...
Blueskyはもともと「自前のドメインをハンドルに使う」ことで、

> このドメインの所有者（関係者）のアカウントだ！

ということを一目で区別できるのが強みでした。好きなドメインを使う権限があるのなら、Bluesky社にお金を払わなくても利用できます。

``@企業の名前.bsky.social`` だと、それが本当に公式なのかはわかりませんが、``@企業の名前.co.jp`` ならば公式だと確認しやすくなるでしょう。

そして2025年から、Blueskyにも「青いチェック（認証バッジ）」が導入されました。Bluesky側が「本物で著名（notable）なアカウント」を認証するほか、Blueskyが認めた外部の組織が認証できる仕組み（Trusted Verifiers）もあります。
...
「Blueskyは分散型なので投稿を削除しても消えない」という話が出回ってるようですが、3分もすれば全サーバーから消えるようです。もちろん、スクショとか撮られたりしたらどうしようもないですが。
...
日本語の検索ですが、ひらがなだけの語句など、うまく検索できないこともあります。ここは改良を待ちたいところです。
...
要望の多かったビデオにも対応しています。2024年の実装当初は「60秒弱」でしたが、現在は最大3分まで投稿できるようになっています。

現在のところ以下のような制限があります（将来変わる可能性はあります）:

- 時間は最大3分まで
- 1つの投稿に1つまで
- 1日に25本まで、かつ、10GBまで

## 保存（Saved Posts / ブックマーク）

2025年9月の v1.108 からに、公式で「保存（Saved Posts / ブックマーク）」が入りました（[TOKIMEKI](https://tokimeki.blue/)などの他のアプリでは以前から実装されていました）。

「いいね」は公開で見えてしまうのに対して、保存は基本的に「自分だけのメモ帳」として使えるので便利です。「あとで読む」「資料として控える」「作業用にキープ」などの目的にどうぞ。ブックマークしたものは「保存済」に入ります。

## DM（チャット）

DM機能があります。メニュー上は「チャット」となっています。
...
知らない相手からのDMは「チャットの要求」という振り分けが行われ、許可／拒否できるようになっています。スパム対策です。

## 友達を見つける

スマホ版の公式アプリ v1.112 で「友達を見つける」機能が使えるようになりました。「すでに知っている人を探す」ための機能です。

Bluesky側は「プライバシー優先」を強調していて、双方が同意してアップロードし、条件が揃ったときだけマッチする（＝一方的に“勝手に探され”にくい）設計だと説明されています。

- 公式の解説→ [Find Your Friends on Bluesky](https://bsky.social/about/blog/12-16-2025-find-friends)

連絡先の取り扱いに抵抗がある人もいると思うので、必要なければ使わなくてOKです（設定からオフにできます）。
...
以前はテスト的に「トレンド」機能が実装されていましたが、現在は「Explore（探索）」の画面に統合されて、

- トレンド（話題）
- スターターパック
- おすすめアカウントやフィードの発見

など、「いま見つけたいもの」への導線としてまとまって扱われることが増えています（UIはアップデートで変わり得ます）。

トレンドが不要なら、設定から表示をオフにできます。オン・オフはアカウントごとの設定というより、端末側の設定に近い扱いです。
...
XではFollowingでもAIで優先付けがされて時系列が乱れることがありますが、Blueskyでは時系列で流れてきます。
...
「Discover」は公式の機能ですが、「For You」というおすすめ投稿を流すフィード（詳しくは後述）もあります。
...
純粋に壁打ちに使うので誰もフォローしたくないとか、静かな人ばかりをフォローしててDiscoverが表示されるのだがDiscoverなんぞみたくない、といった人はFollowingを消し去るか、[Emoji Aquarium](https://bsky.app/profile/emojiaquarium.bsky.social)みたいな無害なアカウントをフォローするとかできます。また、 [Ucho-ten](https://ucho-ten.net/) のような「静かな」クライアントの利用もおすすめできます。
...
なお、Blueskyの仕組み上避けられないのですが、 **誰が誰をブロックしてるのかを調べようとすれば調べられます**。
...
ラベルが適切に設定されてないのではないか、利用規約的に適切な投稿ではないのでは、という投稿を見つけることがあるかもしれません。

そんな投稿があったら、投稿に下の端にある「…」をクリックして表示されるメニューから「投稿を報告」で運営に連絡することができます。

いろんな選択肢が出てきますが、その内容について別記事でまとめました。

[Blueskyの投稿を報告する時の報告理由の一覧](https://whtwnd.com/did:plc:urj4pkche2yvpr5ilj4ykteu/3m623xw62ds2n)
...
Bluesky公式アプリの新リリースの告知を日本語に翻訳してまとめています。

これまでの変更履歴を詳しくみたい方はこちらをどうぞ→ [Bluesky公式アプリ更新情報（非公式）](https://bsky.app/profile/tkusano.jp/feed/193112d9fcdb5e7)
